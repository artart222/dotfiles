...
def find_language():
    result = subprocess.run("setxkbmap -query | grep \"layout:\s\" | awk '{print $2}'", capture_output=True, shell=True)
    result = result.stdout.decode('utf-8')
    result = str(result)
    result = result.strip()
...
keys = [
    ...
    # Screenshots
    Key([], "Print",
        lazy.spawn("/usr/bin/escrotum " + user_home + \
    ...
    Key([mod, "shift"], "Print",
        lazy.spawn("/usr/bin/escrotum -s " + user_home + \
                   "/Pictures/Screenshots/screenshot_%d_%m_%Y_%H_%M_%S.png"),
    ...
    Key([mod, "control"], "Print",
        lazy.spawn("/usr/bin/escrotum -s -C " + user_home + \
                   "/Pictures/Screenshots/screenshot_%d_%m_%Y_%H_%M_%S.png"),
        ...
        top=bar.Bar([
            ...
            widget.TextBox(
                **widget_defaults,
                ...
                decorations=[
                    RectDecoration(
                        colour=theme["dark_blue"], radius=0, filled=True, padding_y=5)
                ]
            ),
            widget.Clock(
                **widget_defaults,
